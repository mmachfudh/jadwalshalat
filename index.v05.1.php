...
$id = isset($_GET['idKota']) ? $_GET['idKota'] : 1;
$sql = "SELECT * FROM luarnegeri WHERE ID = $id";
$qry = $conn->query($sql);
$row = $qry->fetch_assoc();//print_r($row);exit;

if(isset($_GET['days'])) {
      $datePicker = date_parse($_GET['days']);
      // print_r($datePicker);
  }
...
// $latlng  = isset($_GET['latlng']) ? $_GET['latlng'] : '-139.0972,64.0566';
...
$L  =$row['Lintang'];
$H  =$row['Ketinggian'];
$sudutSubuh=isset($_GET['sudutSubuh']) ? $_GET['sudutSubuh'] : 20;
$sudutIsya=isset($_GET['sudutIsya']) ? $_GET['sudutIsya'] : 18;
$KA=isset($_GET['KA']) ? $_GET['KA'] :1;
